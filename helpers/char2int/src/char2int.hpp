...
int undo_char2int(unsigned int *in_buffer_c, int in_buffer_len_i, unsigned char *out_buffer_c, int *out_buffer_len_i)
{
    int result_i = 0;

#ifdef DEBUG_OBFUSCATION1
    printf("undo_char2int: Input       : ");
    for (int i = 0; i < in_buffer_len_i; i++)
    {
...
#ifdef DEBUG_OBFUSCATION1
    printf("undo_char2int: Output  (hex): ");
    for (int i = 0; i < *out_buffer_len_i; i++)
    {
        printf("0x%02X, ", in_buffer_c[i]);
    }
    ...
    printf("undo_char2int: Output (str): ");
    for (int i = 0; i < *out_buffer_len_i; i++)
    {
