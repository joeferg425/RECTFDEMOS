...
int obfuscate3(unsigned char* in_buffer_c, int in_buffer_len_i, unsigned char* out_buffer_c, int* out_buffer_len_i, unsigned char crypto_xor)
{
    int result_i = 0;
...
int deobfuscate3(unsigned char* in_buffer_c, int in_buffer_len_i, unsigned char* out_buffer_c, int *out_buffer_len_i, unsigned char crypto_xor)
{
    int result_i = 0;

    #ifdef DEBUG_OBFUSCATION3
    printf("deobfuscate3: Input       : ");
    for (int i = 0; i < in_buffer_len_i; i++)
    {
        printf("0x%02X, ", in_buffer_c[i]);
    }
    ...
    #ifdef DEBUG_OBFUSCATION3
    printf("deobfuscate3: Output  (hex): ");
    for (int i = 0; i < *out_buffer_len_i; i++)
    {
        printf("0x%02X, ", in_buffer_c[i]);
    }
    ...
    printf("deobfuscate3: Output (str): ");
    for (int i = 0; i < *out_buffer_len_i; i++)
    {
